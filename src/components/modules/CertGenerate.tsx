    ...
    <div className="space-y-5">
      <Card>
        ...
        <div className="flex flex-wrap gap-4 items-center mb-4">
          ...
          <div className="w-full sm:w-auto">
            ...
            <InputNumber min={1} max={10} value={genCount['_base'] || 1} onChange={(v) => setGenCount((p) => ({ ...p, _base: v || 1 }))} className="w-full sm:w-[100px]" addonAfter="份" />
          </div>
          <div className="text-sm text-gray-500 w-full sm:w-auto">当前题库：<b className="text-gray-800">{currentBank?.name || '暂无'}</b> · 已审核 {currentBank?.questions.filter((q) => q.status === 'approved').length || 0} 题</div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div>
              <div className="font-medium">理论考试试卷（Word）</div>
              <div className="text-gray-500 text-sm">基于理论题库抽题并排版生成</div>
            </div>
            <Button type="primary" className="w-full sm:w-auto" onClick={() => handleGenerate('paper', '理论考试试卷')}>生成</Button>
          </div>
          <div className="border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div>
            ...
            <Button type="primary" className="w-full sm:w-auto" onClick={() => handleGenerate('table', '技能考核评分表')}>生成</Button>
          </div>
          <div className="border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div>
            ...
            <Button type="primary" className="w-full sm:w-auto" onClick={() => handleGenerate('answer', '参考答案与评分标准')}>生成</Button>
          </div>
          <div className="border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div>
            ...
            <Button type="primary" className="w-full sm:w-auto" onClick={() => handleGenerate('cert', '职业技能等级认定证书')}>生成</Button>
          </div>
        </div>
      </Card>
      ...
      {certItems.length > 0 && (
        <Card title="已生成的文件">
          ...
          <Table rowKey="id" size="small" columns={columns} dataSource={certItems} pagination={false} scroll={{ x: 520 }} />
        </Card>
      )}
    </div>
  );
